...
export default function NowPlaying() {
  ...
  return (
    <Link
      className="group flex items-center font-medium text-zinc-800 dark:text-zinc-200"
      href={
    ...
    >
      ...
      <div className="ml-4 flex items-center max-w-full truncate">
        {data?.isPlaying && data?.songUrl && <MusicEqualizer />}
        {data?.songUrl ? (
          <p className="ml-2 max-w-max truncate text-sm group-hover:text-spotify-green dark:group-hover:text-spotify-green">
            {data.title}
          </p>
        ) : (
          <p className="text-sm group-hover:text-spotify-green dark:group-hover:text-spotify-green">
            Not playing
          </p>
        )}
        <span className="mx-2 text-sm">{'–'}</span>
        <p className="max-w-max truncate text-sm">
          {data?.artist ?? 'Spotify'}
        </p>
      </div>
    </Link>
