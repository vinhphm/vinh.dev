...
export default function NowPlaying() {
  ...
  return (
    ...
    >
      <svg
        className="h-6 w-6 flex-none fill-zinc-500 transition group-hover:fill-spotify-logo-green"
        viewBox="0 0 168 168"
      >
      ...
      <div className="ml-4 flex max-w-full items-center truncate">
        {data?.isPlaying && data?.songUrl && <MusicEqualizer className="mr-2" />}
        {data?.songUrl ? (
          <p className="max-w-max truncate text-sm text-zinc-800 group-hover:text-spotify-green dark:text-zinc-200 dark:group-hover:text-spotify-green">
            {data.title}
          </p>
        ) : (
          <p className="text-sm text-zinc-700 group-hover:text-spotify-green dark:text-zinc-300 dark:group-hover:text-spotify-green">
            Not playing
          </p>
        )}
        ...
        <p className="max-w-max truncate text-sm text-zinc-500">
          {data?.artist ?? 'Spotify'}
        </p>
      </div>
    </Link>
